...
`expected.toml` (what the gate enforces) and a regenerated `measured.json` + `measured.md`
...
Splitting authority from record removes that path: the gate reads `expected.toml`, so
regenerating `measured.*` cannot change what passes. What remains possible is a hand-edit to
`expected.toml` — and that is the point. It becomes **a one-line diff to a named file with a
comment slot beside it**, rather than a number buried in a regenerated blob.
...
several independent harnesses, nest one level: `<corpus>/<harness>/expected.toml`.
...
- **Nothing regenerates `expected.toml`.** A regeneration script that writes it is a bug.
- **Nobody hand-edits `measured.*`.** A hand-edit there is falsifying a record.

`expected.toml` is TOML because a threshold with no justification beside it is exactly what
gets loosened silently later — and TOML takes comments:

```toml
# validation/<corpus>/expected.toml
[sensitivity]
min = 0.995   # floor agreed in ADR-00XX; record 207's flutter run is the binding case
```
...
3. **The index** — one row per corpus:
   ...
   |---|---|---|---|
   | `<corpus>` | one line | which check reads `expected.toml` | the exact command |

4. **How to add a corpus** — make the directory, write `expected.toml` with a justification
   comment per threshold, run the harness to produce `measured.*`, add the index row, point
   the gate at `expected.toml`.
...
|---|---|---|
| How well does it actually perform, and measured against what? | `validation/<corpus>/` | hand-edited `expected.toml` + regenerated `measured.json`/`.md` |
...
When a measured number moves and the threshold is deliberately relaxed, the `expected.toml`
...
> on `validation/*/expected.toml` (targets the goalpost move, which is the common case) and a
