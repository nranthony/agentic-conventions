...
## Landed (2026-08-26) — #1–#6 and #8

Everything still outstanding is in, with the review's three wording changes and its scope
correction applied. The file edited is the **canonical** `.claude/skills/wrap-up/SKILL.md`,
not the `plugins/myconv/` payload the Scope line named — the payload is regenerated by
`just sync-plugin`, which was run.

- **#1 — §7 is path-agnostic** and is now *§8, "Skills and agent-facing guides (wherever this
  repo keeps them)"*. `.claude/skills/` is named as this blueprint's default rather than a
  requirement; guides are located from the root `AGENTS.md` index. The **floor** the review
  asked for is in the text: if no index resolves them, say so and name the paths checked. The
  frontmatter assumption is gone — where a guide has none, the check degrades to "is the index
  entry still accurate, and does the guide still describe reality?"
- **#2 — §0 has the third state.** Not kept / kept under another name / **could not locate
  (naming the paths checked)**, with the author's phrasing kept: *never configured is a skip;
  present under a different name is a miss*, and report the mapping you inferred.
- **#3 — the tier map prints before §1 and again with the summary.** Open question 3 is
  answered "both", per the review: up front it sets expectations, at the end it is the evidence
  that stops a list of passes reading as full coverage. "A skip is not a pass" appears in both
  places.
- **#4 — new §2, the repo's own change gate.** The review's wording change is *inside* the
  section, not only in the rationale: **enumerate every clause and check each one; do not run
  them.** Open question 1 is answered "wrap-up first"; nothing was added to
  `apply-conventions` in this pass.
- **#5 — new §10, the corrected-fact sweep**, and bounded as the review required: scoped to
  `*.md`, agent instruction files and hand-maintained config comments, explicitly excluding
  vendored trees, lockfiles, fixtures and generated output, and **capped** — a few hits per
  fact, grouped by file, plus a count of what was suppressed. It stays a section, not a skill
  (open question 2).
- **#6 — §4 (ARCHITECTURE.md) covers enumerations**: file lists, counts, command inventories,
  named test suites — "those rot silently because they read as prose".
- **#8 — §11 ranks write-back destinations** ADR > architecture doc > `AGENTS.md` > skill, and
  names the anti-pattern: a durable fact whose only home is a commit message is lost.
- **Open question 4 answered "yes"**, as the review directed: a line under the trigger block
  says the skill is **not a substitute for the repo's own gates** — §2 reads a gate, it never
  runs one.
- **§2's `templates/` exception generalised** from "any example/placeholder tree" to "any
  example, fixture, or test-corpus tree", per the "what is already right" section. The
  exception itself is unchanged, as are the trigger line, `--dry-run`, the apply/propose split
  and §0's "do not propose adding opt-in machinery".

**Sections renumbered 11 → 13** by the two additions; the internal `§10` cross-reference in
*Mode* was updated with them. Nothing was dropped, and nothing here needed to be left out.

**Budget.** The review's ≤150-line budget is superseded by ADR-0015 decision 5 — **200 total
lines**, counted live. `wrap-up` finished at **197/200**, from 142. The headroom is thin on
purpose: the next change to this skill should expect to distil rather than append.
